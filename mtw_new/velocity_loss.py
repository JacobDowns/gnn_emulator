...
class LossIntegral:
    def __init__(self, mesh):
        self.V_mtw = df.FunctionSpace(mesh, "MTW", 3)
        self.V_cg = df.VectorFunctionSpace(mesh, 'CG', 1)
        self.V_dg = df.FunctionSpace(mesh, 'DG', 0)
        
        self.Ubar = Ubar = df.Function(self.V_cg)
        self.Ubar_obs = Ubar_obs = df.Function(self.V_mtw)

        self.w_bar = df.TestFunction(self.V_cg)

        r = Ubar - Ubar_obs

        self.I = df.dot(r, r)*df.dx + df.div(self.Ubar)**2*df.dx 
        self.J_bar = df.derivative(self.I, self.Ubar, self.w_bar)
        self.j_bar = df.Function(self.V_cg)
...
class VelocityLoss(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        ...
        return j_bar*grad_output, None, None, None, None