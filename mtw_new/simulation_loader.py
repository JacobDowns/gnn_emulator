...
class SimulationLoader:
    ...
    def load_features_from_h5(self):
        ...
        for j in range(self.num_steps):
            ...
            S_jump = self.data_mapper.get_jump(vars['B'] + vars['H'])
    ...
    def load_features_from_arrays(self):
        ...
        for j in range(self.num_steps):
            ...
            X = np.load(f'/media/new/gnn_emulator_runs1/{self.i}/X_{j}.npy')
            Y = np.load(f'/media/new/gnn_emulator_runs1/{self.i}/Y_{j}.npy')
            X = torch.tensor(X, dtype=torch.float32)
            Y = torch.tensor(Y, dtype=torch.float32)
    ...
    def save_feature_arrays(self):
        base_dir = f'/media/new/gnn_emulator_runs1/{self.i}'
        for j in range(self.num_steps):
            x = self.Xs[j]
            y = self.Ys[j]
