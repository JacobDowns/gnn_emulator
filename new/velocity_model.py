...
class VelocityModel:
    def __init__(
        ...
        driving_stress = (
                          - omega*df.div(Phibar*H0)*B*df.dx 
                          - omega*df.div(Phibar*H0)*H0*df.dx 
                          + omega*df.jump(Phibar*H0,nhat)*df.avg(B)*df.dS 
                          + omega*df.jump(Phibar*H0,nhat)*df.avg(H0)*df.dS 
                          + omega*df.dot(Phibar*H0,nhat)*B*df.ds 
    ...
    def solve_B_grad(self, B):
        self.B.assign(B)
        self.B_grad_solver.solve()
        return self.B_grad
    
    def solve_S_grad(self, B, H0):
        self.B.assign(B)
        self.H0.assign(H0)
    ...
    def solve_vel(
        ...
        self.solve_S_grad(B, H)
        self.solve_B_grad(B)


        #self.W.sub(0).assign(Ubar0)
        #self.W.sub(1).assign(Udef0)
