...
class Encoder(nn.Module):
    ...
    def forward(self, graph):
        ...
        #print(graph.num_nodes, edges.min(), edges.max(), x_edge.shape[1])
        x_node = torch.zeros((graph.num_nodes, x_edge.shape[1]), device=x_g.device)

        scatter_mean(x_edge, edges[:,0], dim=0, out=x_node)
        scatter_mean(x_edge, edges[:,1], dim=0, out=x_node)
...
class NodeBlock(nn.Module):
    ...
    def forward(self, graph):
        ...
        scatter_mean(x_edge, edges[:,0], dim=0, out=x_node1)
        scatter_mean(x_edge, edges[:,1], dim=0, out=x_node1)

        x_node_ = torch.column_stack([
            x_node,
            x_node1
        ])
...
class Decoder(nn.Module):
    ...
    def __init__(self, hidden_size=32, output_size=1):
        ...
        self.decode_module = build_mlp(hidden_size, hidden_size, output_size, lay_norm=True)
...
class EncoderProcesserDecoder(nn.Module):
    ...
    def __init__(self, message_passing_num, edge_input_size=11, hidden_size=32):
