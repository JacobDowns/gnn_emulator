...
class FDDataLoader:
    ...
    def __init__(self, file_name):
        ...
        with fd.CheckpointFile(file_name, 'r') as afile:
            ...
            dx = self.coords[self.edges[:,0]]
            dy = self.coords[self.edges[:,1]]
            self.dx = dx[:,1] - dx[:,0]
            self.dy = dy[:,1] - dy[:,0]
    ...
    def get_graph(self, j, normalize=True):
        ...
        if normalize:
            input_features /= np.array([0.0247, 0.0284, 0.1312, 0.0738, 0.707, 0.707, 0.0444])
            output_features /= 0.0458
        ...
        # Agregate edge features to get vertex features
        vertex_features = scatter_mean(
            data_j.edge_attr[:,0:4],
            data_j.edge_index[1],
            dim=0,
            dim_size=len(self.coords)
        )
