...
class ResidualModel:
    def __init__(
        ...
        # Function to store residual
        self.R = df.Function(self.W)
        self.dR = df.derivative(R_stress, self.W, self.R)

    def forward(self, B, H, beta2, Ubar, Udef):
        self.residual_model.B.assign(B)
        self.residual_model.H.assign(H)
        self.residual_model.beta2.assign(beta2)
        self.residual_model.W.sub(0).assign(Ubar)
        self.residual_model.W.sub(1).assign(Udef)
        self.B_grad_solver.solve()
        self.S_grad_solver.solve()
        df.assemble(self.R_stress, tensor=self.R)

    def backward(self, B, H, beta2, Ubar, Udef):
        self.forward(B, H, beta2, Ubar, Udef)
        return df.assemble(self.dR)
    

    
class FiredrakeLossModel:
    def __init__(self, mesh):
        residual_model = ResidualModel(mesh)
        self.residual_model = residual_model
        R_vel = self.R_vel = residual_model.R_stress

        # Function to store residual
        self.R = df.Function(residual_model.W)
        self.dR = df.derivative(R_vel, residual_model.W, self.R)
    ...
    def forward(self, B, H, beta2, Ubar, Udef):
        ...
        self.residual_model.W.sub(0).assign(Ubar)
        self.residual_model.W.sub(1).assign(Udef)
        df.assemble(self.R_vel, tensor=self.R)
...
class LossModel:
    def __init__(self, mesh):
        self.loss_model = FiredrakeLossModel(mesh)
        self.residual_model = self.loss_model.residual_model

    @staticmethod
    def forward(self, B, H, beta2, Ubar, Udef):
        self.residual_model.B.dat.data[:] = B
        self.residual_model.H.dat.data[:] = H
        self.residual_model.beta2.dat.data[:] = beta2
        self.residual_model.W.sub(0).dat.data[:] = Ubar
        self.residual_model.W.sub(1).dat.data[:] = Udef
