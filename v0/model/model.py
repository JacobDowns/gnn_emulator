...
class Decoder(nn.Module):
    ...
    def forward(self, graph):
        return self.decode_module(graph.edge_attr)
...
class EncoderProcesserDecoder(nn.Module):
    ...
    def forward(self, graph):
        ...
        # All edge features (len, mx, my, nx, ny, dB, dS)
        X_edge = graph.edge_attr
        
        graph= self.encoder(graph)
        for model in self.processer_list:
            graph = model(graph)
        decoded = self.decoder(graph)
        return decoded #torch.unsqueeze((decoded*n).sum(axis=1), 1)
