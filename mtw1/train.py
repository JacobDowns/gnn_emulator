...
simulator = Simulator(message_passing_num=8, edge_input_size=9, device=device)
...
def train(model:Simulator, train_data, test_data, optimizer):
    ...
    for ep in range(epochs):
        ...
        for j in js:
            ...
            Ubar_obs = y_obs[:,0:3].flatten()
            Udef_obs = y_obs[:,3:].flatten()

            g = g.cuda()
            y = model(g).cpu()

            Ubar_mod = y[:,0:3].flatten()
            Udef_mod = y[:,3:].flatten()

            loss = vel_loss(Ubar_mod, Udef_mod, Ubar_obs, Udef_obs, sim_loader.loss_integral)
