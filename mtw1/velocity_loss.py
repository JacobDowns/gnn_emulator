...
class LossIntegral:
    def __init__(self, mesh):
        ...
        self.j_bar = df.Function(self.V_mt)
        self.j_def = df.Function(self.V_mt)
...
class VelocityLoss(torch.autograd.Function):
    ...
    def forward(ctx, Ubar, Udef, Ubar_obs, Udef_obs, loss_integral):
        ...
        return torch.tensor(df.assemble(loss_integral.I)).clone().detach()
    ...
    def backward(ctx, grad_output):
        ...
        df.assemble(loss_integral.J_def, tensor=loss_integral.j_def)
                             
        j_bar = torch.tensor(loss_integral.j_bar.dat.data[:])
        j_def = torch.tensor(loss_integral.j_def.dat.data[:])
