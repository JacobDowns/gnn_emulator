...
class FiredrakeLossModel:
    def __init__(
        ...
        Lambda = self.Lambda = df.Function(self.V)
        R_full = self.R_full = df.replace(self.R_stress,{self.Psi:Lambda})

        R_adjoint = df.derivative(R_full, W, Psi)
        R_adjoint_linear = df.replace(R_adjoint,{Lambda:dW})
        self.dR = df.lhs(R_adjoint_linear)

        # Function to store residual
        self.R = df.Function(self.W)
        #self.dR = df.derivative(R_stress, self.W, self.W)
