    ...
    def __init__(
        ...
        if velocity_function_space=='MTW':
            ...
            E_vel = self.E_vel = df.FiniteElement(velocity_function_space,
                                                  mesh.ufl_cell(),1)
        elif velocity_function_space=='CG':
            E_cg = df.FiniteElement('CG', mesh.ufl_cell(), 1)
            E_vel = df.MixedElement(E_cg, E_cg)
        ...
        adot = self.adot = df.Function(Q_thk, name='adot') 
        ...
        R_stress = self.R_stress = membrane_stress + basal_stress - driving_stress

        J = self.J = df.derivative(R_stress, self.W)
