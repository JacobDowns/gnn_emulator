...
with fd.CheckpointFile(file_name, 'r') as afile:
    ...
    fd.solve(R == 0, model.W)
    quit()

    for j in range(50):
        r0 = fd.assemble(R)
        J0 = fd.assemble(J).M.handle
        rJ = fd.Function(model.W)

        with r0.dat.vec as  r_p:
            with rJ.dat.vec as rJ_p:
                J0.multTranspose(r_p, rJ_p)
